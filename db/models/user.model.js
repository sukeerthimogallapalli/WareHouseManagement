...
const read = (query, options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    if (options.multi) {
      User.find(query, options.fields)
        .skip(options.skip || 0)
      ...
      User.findOne(query, options.fields || {})
        .lean().exec((err, User) => err ? reject(err) : resolve(User))
    }
  });
};
...
const aggregate = pipe => new Promise((resolve, reject) => {
  User.aggregate(pipe, (err, info) => err ? reject(err) : resolve(info))
});
